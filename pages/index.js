...
const SECRET_HASH =
  '0xbce57ec951304d8c5b3d33595eafc715bfea643cc9281a7936f8ff37bf952bea';
...
//     transition: '0.3s'
// }
const contractAddress = 'TZ5VGgdA87DU6FDTBkcfPMTG4AF4YieZaE'; // Nile //"TXaztL5PnXieEzWWsmBWFpRnFJA7FQ74kc";//'TUNhBUhK5VjrnLs3mvJsA3YhT7cjF1hjCf'TFPEK3yFiMafKGefye8Huq2z7YvEtCWwVB;
let rpsContract;
...
class Index extends Component {
  ...
  async componentDidMount() {
    ...
    await new Promise(resolve => {
      ...
      const timer = setInterval(() => {
        if (tries >= 10) {
          ...
          window.tronWeb = TronWeb(TRONGRID_API, TRONGRID_API, TRONGRID_API);
    ...
    if (!this.state.tronWeb.loggedIn) {
      ...
      window.tronWeb.on('addressChanged', async () => {
        const tmp_tronwebaddress = Utils.tronWeb.address.fromHex(
          (await Utils.tronWeb.trx.getAccount()).address.toString()
        );
    ...
    let CONTRACT_BAL = await tronWeb.trx.getBalance(
      'TRkUGYSyKp2NUsHGZUvSEWTc63Atbv8i7G'
    );
    ...
    if (decimalNum > 0) {
      ...
      if (this.state.allGames.length == 0) {
        ...
        for (let i = 0; i < this.state.allGames.length; i++) {
          ...
          await tronWeb.trx
            .getAccount(this.state.allGames[i].dealer)
            .then(res => {
  ...
  checkIfReveal() {
    ...
    for (let i = 0; i < this.state.allGames.length; i++) {
      if (
        this.state.allGames[i].dealer ==
        tronWeb.address.toHex(this.state.tronwebaddress)
      ) {
  ...
  startCloseListener() {
    Utils.contract.CloseGame().watch((err, { result }) => {
      ...
      if (result) {
        ...
        if (
          result.result == 101 &&
          tronWeb.address.toHex(this.state.tronwebaddress) == result.dealer
        ) {
          swal({
            title: 'Successful',
            text: 'You have Won!',
    ...
    return (
      <div>
        ...
        >
              ...
              if (this.state.allGames.length == 0) {
                ...
                if (this.state.overflowOpen == 'hidden') {
                  this.setState({
                    ...
                    ogOpenText: '-',
                    heightOpen: heightOpenGames,
                    paddingOpen: 20
                  });
                } else {
                  this.setState({
                    overflowOpen: 'hidden',
                    transformOpen: 'scaleY(0)',
                    ogOpenText: '+',
                    heightOpen: 0,
                    paddingOpen: 0
                  });
          ...
          >
            <Typography
              ...
              gutterBottom
              className={'TextStyle'}
              style={{ overflow: 'unset', color: 'white' }}
            >
        ...
        <div
          ...
          style={{
            transform: this.state.transformOpen,
            height: this.state.heightOpen,
            padding: this.state.paddingOpen
          }}
        >
  ...
  revealFunc = async game => {
    try {
      await Utils.contract
        .reveal(game.gameID, 10, SECRET_HASH)
        .send({
  ...
  async letsPlay() {
    try {
      ...
      dealerHash = await Utils.contract
        .getProof(this.state.tronwebaddress, value, SECRET_HASH)
        .call();
      console.log(dealerHash);
      let dealerHashObj = new String(dealerHashObj);
      await Utils.contract
        .createGame(dealerHash, this.state.betAmount * 1000000)
        .send({
          shouldPollResponse: true,
      ...
      <AppBarCustom contractValue={this.state.contractValue}>
        ...
        >
          <div style={{ width: 120, margin: '0px auto' }}>
            <img src="../static/images/rpsLogo.png" style={{ width: 240 }} />
          </div>
          <div className={'MainDiv'}>
            ...
            <div style={{ width: '49%' }}>
              {this.state.hasGame && this.state.letsReveal
                ? this.revealGameGrid(revealGame)
                : this.state.hasGame
